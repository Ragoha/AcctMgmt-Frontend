...
class PgrDialogComponent2 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            open: false,
    ...
    handleDown = (closeType) => {
        const DialogCloseType = {
            CONFIRM: "confirm",
            CANCEL: "cancel",
          };
        this.setState({ open: false });
        if (closeType !== DialogCloseType.CONFIRM) {
            this.props.onClose(DialogCloseType.CONFIRM);
            console.log("닫음버튼");
        } else {
            this.props.onClose(DialogCloseType.CANCEL);
            console.log("취소버튼");
    ...
    handleClickConfirm = async () => {
        if (!this.state.selectedRow.pgrCd || !this.state.selectedRow.pgrNm) {
            ...
            await this.props.handleSetPgrTextField2(emptyRow); // 빈 값의 데이터를 부모로 전달
            this.setState({ selectedRow: emptyRow, keyword: '', pgrRows: [] }); // 선택된 값을 빈 값으로 설정
            this.handleDown();
        } else {
            this.handleDown();
            const selectedRow = this.state.selectedRow;
            await this.props.handleSetPgrTextField2(selectedRow); // 선택된 데이터를 부모로 전달
